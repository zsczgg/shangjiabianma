...
面向多平台电商与仓配业务的商品统一编码系统。系统使用 SQLite 数据库，以商品组织多个库存规格（SKU），每个新规格自动获得 `yyhxfz000001` 格式的永久内部编码，并关联厂家条码、平台商品 ID 与仓配编码。

## 功能

- 商品信息与多规格 SKU 管理
- 自动生成六位流水内部编码，生成后不可修改
- 厂家条码、仓配编码及平台商品 ID 映射
...
应用容器发布到独立端口 `3210`，不占用服务器的 80 默认端口。SQLite 数据库存放在 `./data/shangjiabianma.db`，备份服务每 6 小时生成一次一致性备份并保存在 `./backups/`，默认保留 30 天。

当前访问格式为 `http://服务器IP:3210`。以后绑定域名时，可单独配置反向代理和 HTTPS。
